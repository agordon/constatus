...
PREFIX=/usr
VERSION="2.0"
CXXFLAGS=$(DEBUG) -O3 -march=native -ffast-math -D_FILE_OFFSET_BITS=64 -pedantic -std=c++11 -DNAME=\"$(NAME)\" -DVERSION=\"$(VERSION)\" -O3 `pkg-config --cflags jansson libavformat libswscale libavcodec libavutil libconfig++ cairo exiv2` -I/usr/include/netpbm # -march=native -mtune=native -fomit-frame-pointer -flto
LDFLAGS=$(DEBUG) -ldl -ljpeg -lpng -lcurl -lgwavi `pkg-config --libs jansson libavformat libswscale libavcodec libavutil libconfig++ cairo exiv2` -lswresample -lnetpbm -Wl,--export-dynamic # -flto
